...
func (cfg *config) applierSnap(i int, applyCh chan ApplyMsg) {
	...
	for m := range applyCh {
		if m.SnapshotValid {
			// DPrintf(900, "Installsnapshot %v %v\n", m.SnapshotIndex, lastApplied)
			cfg.mu.Lock()
			// DPrintf(900, "CondInstallSnapshot rafts %v m.SnapshotIndex=%v m.SnapshotIndex=%v\n", i, m.SnapshotIndex, m.SnapshotIndex)
			...
			// DPrintf(900, "apply %v lastApplied %v\n", m.CommandIndex, lastApplied)
			cfg.mu.Lock()
			err_msg, prevok := cfg.checkLogs(i, m)
			cfg.mu.Unlock()
			if m.CommandIndex > 1 && prevok == false {
			...
			if (m.CommandIndex+1)%SnapShotInterval == 0 {
				...
				v := m.Command
				e.Encode(v)
				// DPrintf(900, "Snapshot rafts %v m.CommandIndex=%v\n", i, m.CommandIndex)
