...
func TestFigure8Unreliable2C(t *testing.T) {
	...
	for iters := 0; iters < 1000; iters++ {
		...
		leader := -1
		for i := 0; i < servers; i++ {
			_, _, ok := cfg.rafts[i].Start(nextcmd())
...
func snapcommon(t *testing.T, name string, disconnect bool, reliable bool, crash bool) {
	...
	for i := 0; i < iters; i++ {
		...
		if disconnect {
			...
			DPrintf(900, "disconnect %v\n", victim)
			tcmd := rand.Int()
			DPrintf(900, "after disconnect tester put cmd %v\n", tcmd)
			cfg.one(tcmd, servers-1, true)
		}
		if crash {
			cfg.crash1(victim)
		...
		if disconnect {
			...
			DPrintf(900, "reconnect %v\n", victim)
			tcmd := rand.Int()
			DPrintf(900, "after reconnect tester put cmd %v\n", tcmd)
			cfg.one(tcmd, servers, true)
			leader1 = cfg.checkOneLeader()
		}
		if crash {
