...
func TestSnapshot(t *testing.T) {
	...
	for i := 0; i < n; i++ {
		check(t, ck, ka[i], va[i])
		DPrintf(800, "phase 1 %v", i)
	}
	DPrintf(800, "join 1 join 2 leave 0")
	cfg.join(1)
	cfg.join(2)
	cfg.leave(0)
...
func TestChallenge1Delete(t *testing.T) {
	...
	for gi := 0; gi < cfg.ngroups; gi++ {
		for i := 0; i < cfg.n; i++ {
			...
			total += raft + snap
			DPrintf(800, "raftsize=%v snapsize=%v", raft, snap)
		}
	}
